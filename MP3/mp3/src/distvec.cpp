...
public:
    ...
    void distanceVector(int start) {
        distances.assign(nodes.size(), INF);
        distances[start] = 0;
        predecessors.assign(nodes.size(), -1);
        vector<int> exploreNodes;
        // |V| - 1 times iterations
        exploreNodes.push_back(start);
        for (int i = 0; i < nodes.size()-1; i ++){
            vector<int> nextExploreNodes;
            while (!exploreNodes.empty()){
                int currentNodeIdx = exploreNodes.back();
                exploreNodes.pop_back();
                for (auto neighbor : nodes[currentNodeIdx].neighbors) {
                    if (distances[currentNodeIdx] + neighbor.first < distances[neighbor.second]){
                        distances[neighbor.second] = distances[currentNodeIdx] + neighbor.first;
                        predecessors[neighbor.second] = currentNodeIdx;
                        nextExploreNodes.push_back(neighbor.second);
                    }
                    // trash condition of choosing the smaller vertex in the path.
                    else if (distances[currentNodeIdx] + neighbor.first == distances[neighbor.second]){
                        if (predecessors[neighbor.second] > currentNodeIdx) {
                            distances[neighbor.second] = distances[currentNodeIdx] + neighbor.first;
                            predecessors[neighbor.second] = currentNodeIdx;
                            nextExploreNodes.push_back(neighbor.second);
                        }
            ...
            exploreNodes.assign(nextExploreNodes.begin(),nextExploreNodes.end());
        }
        return;
    }
    ...
    void printMsgTrans(ofstream& outFile){
        ...
        while (currentMsgInfo != nullptr){
            ...
            distanceVector(currentMsgInfo->src);
